...
class Pessoa():
    def __init__(self, nome_p = '', fone1 = '', fone2 =''):
        ...
        self.sv = Button(self.frame, width=10, text='SALVAR', fg='green', command=self.salvar)
        self.sv.place(x=80, y=10)

        self.bs = Button(self.frame, width=10, text='BUSCA', fg='green', command=self.busca)
        self.bs.place(x=190, y=10)

        self.al = Button(self.frame, width=10, text='ALTERAR', fg='green', command=self.alterar)
        self.al.place(x=280, y=10)

        self.dl = Button(self.frame, width=10, text='EXCLUIR', fg='green', command=self.delete)
        self.dl.place(x=390, y=10)

        self.sr = Button(self.frame, width=25, text='SAIR', fg='red', command=self.frame.destroy)
        self.sr.place(x=250, y=300)

    def salvar(self):
        self.nome_p = str(self.n_nome.get())
        self.nome_p = self.nome_p.capitalize()
        if self.nome_p.isalpha()==False:
            mb.showerror('Incorreto','No nome so podem haver letras!')
            self.n_nome.delete(0, END)
        try:
            self.fone1 = int(self.n_fone1.get())
            self.fone2 = int(self.n_fone2.get())
        except ValueError:
            mb.showerror('Incorreto','Numeros de telefone so podem ser inteiros')
